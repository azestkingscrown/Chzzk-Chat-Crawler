...
![Chzzk Chat Crawler 실행 화면](https://azestkingscrown.cloud/uploads/1780033844188-screenshot_2026-05-29_14-50-13.png)
...
A1: 스트리머가 해외시청차단을 설정 한 경우 이 경우엔 한국 IP를 이용하시면 해결 됩니다.
해외 IP를 사용하시는 분께서는 IP우회를 안 하실 경우 아쉽게도 스트리머가 제한을 풀지 않는 한 불가능 합니다.

A2: 스트리머가 해외시청차단을 하지 않아도 스트리머가 설정한 카테고리에 [같이보기]가 있다면 저작권 상 수집이 불가능 합니다.

A3. 미성년자 시청 제한 방송은 수집이 안 됩니다.

Q: 닉네임으로 등록했는데 다른 사람이 등록돼요.

A: 검색 결과 중 인지도가 가장 높은 채널을 우선적으로 가져옵니다. 만약 결과가 다르다면 스트리머의 32자리 고유 ID를 입력해 주세요.
